...
for p = 3:length(DIR)
    if DIR(p).isdir == 1
        projectName = DIR(p).name;
        cd(projectName);
        load("spot_struct.mat");
        cd ..
        master_struct(counter).spot_struct = spot_struct;
        master_struct(counter).projectName = projectName;
        clear spot_struct
        counter = counter+1;
    end
end
